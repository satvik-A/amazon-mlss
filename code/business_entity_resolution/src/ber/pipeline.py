...
candidates(S, R, S_all, nz)  ->  annotated pool [s1, r, sc, prk, xrk, a0.., exp, gid, rel, rsc, rbest, rev_margin]
  ...
  4. reverse preference: which S1 of the whole country each record prefers (index over ALL S1s)
...
def candidates(S_raw: pl.DataFrame, R_raw: pl.DataFrame, S_all_raw: pl.DataFrame, nz, log=print, keep_prk: int = 60,
    ...
    # reverse preference over ALL S1s of the country
    same = S_all_raw.height == S_raw.height and S_all_raw["entity_id"].equals(S_raw["entity_id"])
    S1n = Q if same else norm_chunks(nz, S_all_raw).with_columns(pl.Series("id", np.arange(S_all_raw.height, dtype=np.uint32)))
    ...
    if same:
        samp = pl.DataFrame({"sall": np.arange(S_raw.height, dtype=np.uint32), "sq": np.arange(S_raw.height, dtype=np.uint32)})
    else:
