...
- [x] Zero-shot on 6k eval pairs (C-split S1, top-15 + aux candidates, 7.9% positive): **bge-reranker-v2-m3 AUC 0.992** (US 0.992, India 0.995; top-1 on non-singletons 0.998), **Qwen3-Reranker-0.6B 0.980**; blocking score alone 0.941. bge fine-tune: 43 pairs/s on a T4; scoring ~260 pairs/s.
- [x] **bge-reranker-v2-m3 fine-tuned** (272k pairs, 0.91 epoch, 105 min on one T4): eval AUC **0.9997** (US 0.9997, India 0.9997), logloss 0.024, top-1 on non-singletons 0.998; but for 22% of singletons the best candidate still has p ≥ 0.5 (twin decoys) → stacking with the GBDT's number features + has-match head is the lever. Scoring 250 pairs/s.
- [x] **Qwen3-Reranker-4B** (LoRA, fp16 base, grad checkpointing): zero-shot AUC 0.9907; after only 18.6k training pairs (6% epoch, 3 pairs/s on a T4) AUC **0.99954** on a 45k-pair eval subset, top-1 0.999 — size helps, but scoring is 11 pairs/s → only for a small uncertain band. Qwen3-Reranker-0.6B: zero-shot 0.980; LoRA training OOM at batch 24 in fp32 (needs checkpointing / batch 12) — deprioritised (bge is better and 5x faster).
- [x] **Qwen3.5-2B** (hybrid Gated-DeltaNet, LoRA, fp16): zero-shot 0.984 → AUC 0.99956 / logloss 0.0226 after 28k pairs (same eval as bge: 0.9997 / 0.0239) — strongest per training pair, best singleton behaviour; 16 pairs/s scoring → uncertain-band judge candidate. Qwen3.5-0.8B zero-shot 0.983.
- [x] **v4 (1M pairs):** MuRIL-base AUC 0.99969, top-1 0.99974, 838 pairs/s; CANINE-c 0.99937, 530 pairs/s.
- [x] Kaggle GPU pitfalls: Qwen full-sequence vocab logits OOM → `logits_to_keep=1`; image torchao 0.10 breaks peft LoRA → uninstall torchao; 4B scoring batch 16. Max 2 concurrent GPU sessions, 5 CPU; TPU v3-8 VM usable as a large CPU host (queue).
...
- [x] ~~France transfer gap~~ **resolved: not a model gap.** France pseudo-pair hit rate 95.1% (vs 99.5–99.95%), but the France probe (`er-france-probe`, one shard, 84k pseudo-pairs) shows 98% of the 3,852 "missed" pairs belong to ANOTHER same-name S1 (the record's best S1 is a different S1; median same-name S1 count 89 vs 2 for hits; blocking score ~0): generic French names ("Lille Sportive", "Nantes Amis") at the same house number on different streets, where city + region/department synonyms supply the ≥3 shared address words. Possible real misses: 79 = **0.09%** of France pseudo-pairs. → France transfer looks fine. Pseudo-pair rule needs a uniqueness condition (record's best S1 / low name frequency) before it is used for France self-training.
