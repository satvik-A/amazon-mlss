"""[pass 4, one country per job: ONLY] Level-1 test scoring: pool -> cut-off (candidate_pairs) -> features -> matcher p.
Writes level1_test_<country>_<k>.parquet (features + p, for the test cross-encoder scoring and level 2), cands_<country>.parquet
and level-1 matches matches_l1_<country>.parquet; er-final4 merges the countries into the submission files.
...
cands, matches = [], []
NSH = int(os.environ.get("NSH", 1 if LOCAL else 12))   # S1 shards per country: features for 70M+ rows do not fit at once
for p in POOLS:
