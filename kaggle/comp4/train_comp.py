...
def f05(sel, d):
    ids = d["s1"].unique(); return macro_f05(sel.select("s1", "r"), gt_all.filter(pl.col("s1").is_in(ids.implode())), ids)
def dec(d, col, t1, t2):
...
def tune(d, col):
    g = [(a, b) for a in np.arange(0.3, 0.91, 0.05) for b in np.arange(0.3, 0.91, 0.05)]
    return max(((a, b, f05(dec(d, col, a, b), d)["f05"]) for a, b in g), key=lambda z: z[2])
...
res = {}
for col in ("p2", "p3"):
    a, b, fb = tune(Tu, col); r = f05(dec(C, col, a, b), C)
    res[col] = dict(t1=float(a), t2=float(b), F_tune=fb, C=r["f05"], P=r["pair_precision"], R=r["pair_recall"])
    print(col, res[col])
d = res["p3"]["C"] - res["p2"]["C"]
print(f"DELTA competition layer minus stack on C: {d:+.5f} -> {'ACCEPT' if d >= 0.001 else 'REJECT'}")
m.save_model(f"{O}/comp.txt")
json.dump(dict(features=feats, t1=res["p3"]["t1"], t2=res["p3"]["t2"], C=res["p3"]["C"], C_stack=res["p2"]["C"], delta_C=d), open(f"{O}/comp.json", "w"), indent=1)
