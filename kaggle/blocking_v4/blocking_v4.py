...
T0 = time.time(); WD = "." if LOCAL else "/kaggle/working"; OUT = open(f"{WD}/results_v4.txt", "w")
def log(*a):
...
def tokens(p, idcol, query=False):
...
def retrieve(Sp, Rt, dfc, chunk=1500):
    ...
    for c0 in range(0, Sp.height, chunk):
        ...
        f = a.group_by(["id", "id_r"]).agg(pl.col("rk").filter(pl.col("arm") == 0).min().alias("prk"),
                                           ...
                                           *[(pl.col("arm") == k).any().alias(f"a{k}") for k in range(6)])
...
Pos = g.filter(pl.col("s1").is_in(samp["entity_id"].implode())).join(Rraw, on="r") \
       ...
       .join(C.select("s1", "r", "frk", "prk", "xrk", *[f"a{k}" for k in range(6)]), on=["s1", "r"], how="left") \
...
log("\n=== UNION recall: primary topK + aux(top15/20 each) + sibling expansion ===")
for K in (20, 30, 50, 100):
    nc = C.filter((pl.col("prk") <= K) | (pl.col("xrk") <= 20)).height / samp.height + float(exp_per.sum()) / samp.height
    ...
    log("  ".join(row))
    e = Pos.group_by("s1").agg(((pl.col("prk") <= K) | (pl.col("xrk") <= 20) | pl.col("exp")).all().alias("ok"))
...
for k, nme in [(0, "primary"), (3, "namepair"), (4, "keys"), (5, "nameonly")]:
    oth = [f"a{j}" for j in (0, 3, 4, 5) if j != k]
...
miss = Pos.filter((pl.col("prk") > 30) & (pl.col("xrk") > 20) & ~pl.col("exp")).join(R.select(pl.col("entity_id").alias("r"), "business_name", "business_address"), on="r") \
...
for c in (["France"] if not LOCAL else []):
    ...
    vol = cand.filter(pl.col("frk") <= 30).group_by("id").agg(pl.len().alias("n"), (pl.sum_horizontal(*[pl.col(f"a{k}").cast(pl.Int8) for k in (0, 3, 4, 5)]) >= 2).sum().alias("strong"))
