...
T0 = time.time(); WD = "." if LOCAL else "/kaggle/working"; OUT = open(f"{WD}/results_v3.txt", "w")
def log(*a):
...
def prep(df):
    ...
    return df.select("entity_id", pl.col("country").cast(pl.Categorical),
        ...
        ad.str.extract_all(r"\b[a-z]{1,3}[-/ ]?\d+[a-z]?\b").list.eval(pl.element().str.replace_all(r"[-/ ]", "").str.replace(r"^([a-z]+)0+", "$1")).alias("au"))

def skel(e):
    """consonant skeleton of a lowercase ascii word expression (vectorized)"""
    e = e.str.replace_all(r"c([eiy])", "s$1").str.replace_all("z", "s").str.replace_all("ksh", "x").str.replace_all("ph", "f").str.replace_all(r"([bcdgjkpstz])h", "$1") \
         .str.replace_all("ck", "k").str.replace_all("q", "k").str.replace_all("c", "k").str.replace_all("w", "v")
...
def retrieve(Sp, Rt, dfc, chunk=1500):
    ...
    for c0 in range(0, Sp.height, chunk):
        ...
        f = a.group_by(["id", "id_r"]).agg(pl.col("rk").filter(pl.col("arm") == 0).min().alias("prk"),
                                           pl.col("rk").filter(pl.col("arm") != 0).min().alias("xrk"),
                                           *[(pl.col("arm") == k).any().alias(f"a{k}") for k in range(5)])
        f = f.with_columns(pl.when(pl.col("prk").is_not_null()).then(pl.col("prk").cast(pl.Float64))
                             .otherwise(1000 + pl.col("xrk").cast(pl.Float64)).alias("key"))
        res.append(f.with_columns(pl.col("key").rank("ordinal").over("id").alias("frk")))
...
Pos = g.filter(pl.col("s1").is_in(samp["entity_id"].implode())).join(Rraw, on="r") \
       ...
       .join(C.select("s1", "r", "frk", "prk", "xrk", *[f"a{k}" for k in range(5)]), on=["s1", "r"], how="left") \
       .join(E, on=["s1", "r"], how="left").with_columns(pl.col("exp").fill_null(False), pl.col("frk").fill_null(10**9), pl.col("prk").fill_null(10**9), pl.col("xrk").fill_null(10**9))
def rec(h, K, withexp=False):
    if h.height == 0: return float("nan")
    m = (h["frk"] <= K) | (h["exp"] if withexp else False)
    return float(m.mean())
log("\n=== v3 recall (pairs) by fused rank K, and with sibling expansion (+exp from top-10) ===")
...
def rec_union(h, K):
    if h.height == 0: return float("nan")
    return float(((h["prk"] <= K) | (h["xrk"] <= 15) | h["exp"]).mean())
log("\n=== UNION recall: primary topK + aux(top15 each) + sibling expansion ===")
for K in (20, 30, 50, 100):
    nc = C.filter((pl.col("prk") <= K) | (pl.col("xrk") <= 15)).height / samp.height + float(exp_per.sum()) / samp.height
    row = [f"K={K:3d} union={rec_union(Pos, K):.4f} (cands/S1={nc:.1f})"]
    for sl in ["indic", "noaddr", "web", "alias"]:
        row.append(f"{sl}={rec_union(Pos.filter(pl.col(sl)), K):.3f}")
    for c in ["US", "India"]:
        row.append(f"{c}={rec_union(Pos.filter(pl.col('country') == c), K):.4f}")
    log("  ".join(row))
    e = Pos.group_by("s1").agg(((pl.col("prk") <= K) | (pl.col("xrk") <= 15) | pl.col("exp")).all().alias("ok"))
    log(f"      entity-complete: {e['ok'].mean():.4f}")
...
for k, nme in [(0, "primary"), (3, "namepair"), (4, "keys")]:
    oth = [f"a{j}" for j in (0, 3, 4) if j != k]
...
miss = Pos.filter((pl.col("prk") > 30) & (pl.col("xrk") > 15) & ~pl.col("exp")).join(R.select(pl.col("entity_id").alias("r"), "business_name", "business_address"), on="r") \
...
vol = C.filter(pl.col("frk") <= 30).group_by("s1").agg(pl.len().alias("n"), (pl.sum_horizontal(*[pl.col(f"a{k}").cast(pl.Int8) for k in (0, 3, 4)]) >= 2).sum().alias("strong"))
...
for c in (["France"] if not LOCAL else []):
    ...
    vol = cand.filter(pl.col("frk") <= 30).group_by("id").agg(pl.len().alias("n"), (pl.sum_horizontal(*[pl.col(f"a{k}").cast(pl.Int8) for k in (0, 3, 4)]) >= 2).sum().alias("strong"))
    log(f"{c}: mean candidates(top30) {vol['n'].mean():.1f}; mean 'strong' (>=2 arms) {vol['strong'].mean():.2f}; exp/S1 {ex.height/Sc.height:.2f}")
